...
Steps 1-6 of plex_rename.py only move *files*. They never carry Plex *user data*
-- watched/unwatched, play counts, resume positions, user ratings. This module
migrates that into Jellyfin over its REST API (version- and backend-agnostic,
works on any 10.11.x and even a Postgres backend, server stays running).
...
# =========================================================================== #
# Connection / onboarding (mirrors plex_rename.connect)
# =========================================================================== #
API_KEY_STEPS = (
...
class MigratedLog:
    ...
    been added, so a re-run doesn't double-count. Stored as a small JSON list."""
    ...
    def record(self, user_id, item_id):
        self._pairs.add((str(user_id), str(item_id)))
        self._flush()

    def _flush(self):
        with open(self.path, "w", encoding="utf-8") as f:
            json.dump(sorted(self._pairs), f)
